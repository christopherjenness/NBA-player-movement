...
class loaded(object):
    ...
    def __init__(self):
        ...
        """
        self.date = a.date
        self.home_team = a.home_team
        self.away_team = a.away_team
        self.tracking_id = '{self.date}.{self.away_team}.at.{self.home_team}'.format(self=self)
        self.tracking_data = a.tracking_data
        self.game_id = a.game_id
        self.pbp = a.pbp
        self.moments = a.moments
        self.player_ids = a.player_ids
        self.away_id = self.moments.ix[0].positions[6][0]
        self.home_id = self.moments.ix[0].positions[1][0]
        self.team_colors = {-1: "orange",
                            self.away_id: "blue",
    ...
    def watch_play(self, game_time, length, highlight_player=None, convex_hull=False):
        """
        Method for viewing plays in game.  
        Outputs video file of play in {cwd}/temp
        ...
        for frame in range(starting_frame, ending_frame):
            self.plot_frame(frame, highlight_player=highlight_player, convex_hull=convex_hull)
        command = 'ffmpeg -framerate 20 -start_number {starting_frame} -i %d.png -c:v libx264 -r 30 -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" {game_time}.mp4'.format(starting_frame=starting_frame, game_time=game_time)
        os.chdir('temp')
        os.system(command) 
        os.chdir('..')
    ...
    def plot_frame(self, frame_number, highlight_player=None, convex_hull=False):
        """
        ...
        plt.scatter([30, 67], [2.5, 2.5], s=100, 
                    c=[self.team_colors[self.away_id], self.team_colors[self.home_id]])
        if convex_hull == 'home':
            points = np.column_stack((x_pos[1:6], y_pos[1:6]))
            hull = ConvexHull(points)
            plt.fill(np.array(x_pos[1:6])[hull.vertices], np.array(y_pos[1:6])[hull.vertices],
                     color=self.team_colors[self.home_id], alpha=0.3)
        if convex_hull == 'away':
            points = np.column_stack((x_pos[6:], y_pos[6:]))
            hull = ConvexHull(points)
            plt.fill(np.array(x_pos[6:])[hull.vertices], np.array(y_pos[6:])[hull.vertices],
                     color=self.team_colors[self.away_id], alpha=0.3)
        plt.savefig('temp/{frame_number}.png'.format(frame_number=frame_number),bbox_inches='tight')
        plt.close()
        return self
...
b = loaded()
#b.watch_play(88, 5, convex_hull='home')
...
for frame in range(len(b.moments)):
    ...
    if offensive_team:
        home_area, away_area = b.get_spacing_area(frame)
        home_areas.append(home_area)
        away_areas.append(away_area)
    if frame % 1000 == 0:
        print(frame)
